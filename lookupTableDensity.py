...
def getTopKNeighborsPWM(freqDict, prot, neighborDict, decomp, topk,
                        skipExact, verbose = None, canSeqWeights = None,
                        maxDensity = None):
	...
	for k in neighborDict.keys():
		...
		#### Code for density weighting #####
		# Adjust the weight according the the frequency of the 
		# neighbors across the dataset if desired
		if canSeqWeights != None:
			totWeight = 0.0
			for i, n in enumerate(neighborDict[k]):
				
				# Stop if we've found the maximum density we want
				if totWeight >= maxDensity:
					break
				
				# Adjust the weight for this neighbor
				if canSeqWeights.has_key(n):
					nWeights[i] = nWeights[i] * canSeqWeights[n]
					totWeight += canSeqWeights[n]
		
			# Lop off any neighbors beyond the maximum density of 
			# neighbors allowed for averaging over
			neighborDict[k] = neighborDict[k][:i]

		####### End code for density weighting ###########
			...
			for i in range(len(baseVectors)):
				...
				if verbose != None:
					...
					searchStr = '"%s%s%s%s"'%(neighborDict[k][i][0],
					                            ...
					                            neighborDict[k][i][2],
					                            neighborDict[k][i][3])
					handle = os.popen("grep %s %s*_4pos.txt" %(searchStr, dirname))
					for line in handle:
						print line.strip()
						#sp_line = line.strip().split(':')
						#file = sp_line[0].split('/')[-1]
						#line_info = '  '.join(sp_line[1].split('\t')[:2])
						#if file == 'all.txt':
						#	continue
...
def get3merList(freqDict, protein, canonical = False,
				...
				verbose = None, canSeqWeights = None,
				maxDensity= None):
	...
	return getTopKNeighborsPWM(freqDict, protein, neighborDict, decompose,
	                           topk, skipExact, verbose, canSeqWeights,
	                           maxDensity)
...
def lookupCanonZFArray(freqDict, canonZFs, useNN = True, 
                       ...
                       verbose = None, canSeqWeights = None, maxDensity=None):
...
def lookupCanonZF(freqDict, canonProt, useNN = True, skipExact = False,
	...
	if not useNN:
		...
		targList, neighborsPerBase = get3merList(freqDict, canonProt, canonical,
	                                         useNN, skipExact, decompose, topk, 
	                                         verbose, canSeqWeights, maxDensity)
...
def getCanSeqWeights(fname):
	# create a dictionary mapping the existing canonical sequences
	# to their frequency across the entire dataset

	fin = open(fname, 'r')
	header = fin.readline().strip().split()
	wInd = header.index("weight")
	pInd = header.index("prot")

	weightDict = {}
	for line in fin:
		sp_line = line.strip().split()
		canProt = sp_line[pInd]
		weight = sp_line[wInd]
		weightDict[canProt] = float(weight)
...
def predictTop64(fname, maxDensity = None):
	...
	# Get the dictionary of canonical sequence weights
	canSeqWeights = getCanSeqWeights(inDir + 'bindVectsUnique4pos.txt')
	#for k in sorted(canSeqWeights.keys()):
	#	print k, canSeqWeights[k]
	...
	# Do the lookup or nearest neighbor
	outpath = dirpath + newDir
