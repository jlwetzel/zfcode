...
def addUpCounts(natZFDict, keys):
...
def getNatZFDict(path):

    natZFDict = {}
    fin = open(path, 'r')
    for line in fin:
        sp_line = line.strip().split()
        natZFDict[sp_line[0]] = eval(sp_line[1])
...
def computeBindingDiversity(proteinDir):
    ...
    hsapZFDict = getNatZFDict('../data/hsapZFs/Hsap_zf_domains_2013.txt')
    hsapZFSet = set(hsapZFDict.keys())
    compareToNaturalUnique(bindingSets4, hsapZFSet, 'Human', fings, strins, bothStrinsUnion)
    compareToNaturalCounts(bindingSets4, hsapZFDict, 'Human', fings, strins, bothStrinsUnion)
